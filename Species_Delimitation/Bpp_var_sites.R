...
#Based on scripts available in Huang, 2018; Tonzo et al., 2019

library(pegas);

data <- scan('BPP.txt', what = 'character', sep = '\n'); #input generated by BPP_Input.R
id.lines <- grep('^\\d+[[:space:]]\\d+', data); 
working.lines <- c(id.lines, length(data) + 1);
...
for(i in 1:length(id.lines)){
  ...
  if(start.line != end.line){
    ...
    for(ii in 1 : length(work.range) - 1){
      ...
      seq.1 <- sub(s.pattern, rep.pattern, data[work.range[ii]]);
      seq.2 <- sub(s.pattern, rep.pattern, data[work.range[ii + 1]]);		
    ...
    temp.theta <- (theta.s(temp.var.site, n.ind.seq))/110;
...
hist(theta.vec, xlim = c(-0.01, 0.05), xlab = 'Theta.s/Sites');
...
lower.limit <- 5; #at least how many var sites in a alignment
upper.limit <- 25; #at most how many var sites are allowed in an alignment
selected.loci <- which(n.var.sites > lower.limit & n.var.sites < upper.limit);
chose.loci <- sample(selected.loci, 200); #randomly choose 100 loci from the selected loci that have desired number of var sites
...
for(i in 1:length(chose.loci)){
  ...
  temp.bpp.file <- data[temp.wanted.lines];
  write.table(temp.bpp.file, append = TRUE, file='L200_25.txt', quote = FALSE, sep = '\n', row.names = FALSE, col.names = FALSE);#change the file name here
}
